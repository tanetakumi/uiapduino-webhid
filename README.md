...
### 1a. ファームを取得（Release から・推奨）

ビルド環境がなくても利用できます。

1. [GitHub Releases](https://github.com/tanetakumi/uiapduino-webhid/releases/latest) から **`webhid.bin`** をダウンロード
2. [docs/flashing.md](docs/flashing.md) の手順で Web Flasher から書き込み
...
[docs/flashing.md](docs/flashing.md) を参照してください（Release 版・ビルド版のどちらも同じ手順です）。

要約:

1. 書き込み待機モード（`VID_1209&PID_B803`）
2. [UIAP Web Flasher](https://yuukiumeta-uiap.github.io/rv003usb-webflasher/example.html) で `webhid.bin` を書き込み
3. USB 抜き差し → 通常モード（`PID D011`）
...
| 含む | 含まない（`.gitignore` / 別配布） |
|------|----------------------------------|
| `firmware/`, `web/`, `scripts/`, `docs/` | `external/ch32fun`, `external/rv003usb` |
| ドキュメント（本 README 等） | ビルド成果物 `*.bin`, `*.hex`, `*.elf` 等（[Releases](https://github.com/tanetakumi/uiapduino-webhid/releases) で配布） |
...
表示は Web UI の「OLED テキスト」から行います（ファーム ver `0x04` 推奨、`0x03` 以上で基本機能可）。OLED を接続しなくても LED と pin 9 ボタンは動作します。起動直後は画面はクリアされた状態です。
...
| `0x12` | OLED_REINIT | I2Cバスを再初期化し、OLEDを再検出（v4） |

`OLED_LINE`は印字可能なASCII（`0x20`–`0x7E`）だけを表示し、それ以外を空白に置換します。NUL以降とbyte18–31は表示に使用しません。

### ステータス（Input Report）— プロトコル v4

| オフセット | 意味 |
|------------|------|
| 0 | LED 状態（`0` = OFF, `1` = ON） |
| 1 | プロトコルバージョン（現在 **`0x04`**） |
| 2 | ボタン状態（`0` = 開放, `1` = 押下） |
| 3 | イベントフラグ bit0 = 押下エッジ, bit1 = 解放エッジ |
| 4 | OLED状態（`0` = 未接続/初期化失敗、`1` = 表示可能） |
| 5..7 | 予約 |

プロトコルv4はv3のLED・ボタン・OLEDコマンドと互換です。v4では `OLED_REINIT`（`0x12`）により、I2Cエラー後にUSB抜き差しなしでOLEDを再検出できます。Web UI は再接続時に入力欄の内容を自動で再送信します（オプションで無効化可）。
...
| OLEDが途中から無効になった | 配線を確認し、Web UI の **OLEDを再検出** を試す（ver `0x04` 以上）。直らなければ USB 抜き差し |
